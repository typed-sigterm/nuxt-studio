...
::warning
Custom providers run in the browser and are inlined at build time. Avoid relying on closure state or server-only modules, and use the `accessToken` passed by Studio when needed.
::
