...
Ensure your custom provider is safe to run in the browser and uses the `accessToken` passed by Studio when needed. Custom provider functions are inlined at build time, so avoid relying on closure state or server-only modules.
